...
public class BlueCapeRenderFeature <T extends LivingEntity, M extends EntityModel<T>> extends ElytraFeatureRenderer<T, M> {
    private static final Identifier BLUE_CAPE = new Identifier(JonathansBatsuits.MOD_ID, "textures/entity/blue_cape_model.png");

    private final ElytraEntityModel<T> elytra;

    public BlueCapeRenderFeature(FeatureRendererContext<T, M> context, EntityModelLoader loader) {
    ...
    public void render(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, T livingEntity, float f, float g, float h, float j, float k,
        ...
        if (itemStack != null && itemStack.getItem() == ModItems.BLUE_CAPE && itemStack.getItem() instanceof FabricElytraItem) {
            Identifier identifier = livingEntity instanceof AbstractClientPlayerEntity ? ((abstractClientPlayerEntity =
                    ...
                    abstractClientPlayerEntity.getElytraTexture() : BLUE_CAPE) : BLUE_CAPE;
            matrixStack.push();
